...
func NewBotAPI(token string) (*BotAPI, error) {
	//dial := net.Dial
	//dialer, err := proxy.SOCKS5("tcp", "nl4.proxy.veesecurity.com:80", &proxy.Auth{User: "PROXY_5ACFDD47A2035", Password: "170cde1138e29b8c"}, proxy.Direct)
	//
	//if err != nil {
	//	panic(err)
	//}
...
func (bot *BotAPI) MakeRequest(endpoint string, params url.Values) (APIResponse, error) {
	...
	if resp.StatusCode == http.StatusForbidden {
		fmt.Println("||| MakeRequest 2")

		return APIResponse{}, errors.New(ErrAPIForbidden)
	}

	if resp.StatusCode != http.StatusOK {
		fmt.Println("||| MakeRequest 3")
		//fmt.Println("||| MakeRequest resp = ", resp)
		//fmt.Printf("||| MakeRequest resp = %+v\n", resp)
		//fmt.Printf("||| MakeRequest resp.Body = %+v\n", resp.Body)
		//fmt.Printf("||| MakeRequest resp.Request = %+v\n", resp.Request)
		//fmt.Printf("||| MakeRequest resp.TLS = %+v\n", resp.TLS)
...
func (bot *BotAPI) Send(c Chattable) (mes Message, err error) {
	defer func() {
		recover()
	}()
	switch c.(type) {
	case Fileable:
		mes, err = bot.sendFile(c.(Fileable))
		return mes, err
	default:
		mes, err = bot.sendChattable(c)
		return mes, err
	}
}
